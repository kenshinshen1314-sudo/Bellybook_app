...
export const Card: React.FC<{
  ...
  return (
    <div onClick={onClick} className={`${styles.bgCard} ${styles.border} rounded-2xl shadow-sm overflow-hidden ${className}`}>
      {children}
    </div>
  );
};
...
export const TabBar: React.FC<{
  ...
  const textActive = styles.textTitle.replace('/90', ''); // 移除透明度用于 Tab
...
export const NavBar: React.FC<{
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 h-[44px] mt-safe-top z-40 flex items-center justify-between px-4 ${className}`}>
      ...
      <div className={`${styles.textTitle} font-semibold text-lg truncate`}>{title}</div>
...
export const ListItem: React.FC<{
  ...
  const styles = useThemeStyles(theme);
  const bgClass = `${styles.bgCard} ${styles.border} active:bg-[var(--muted)]`;
  ...
  return (
    ...
    >
      <div className="flex items-center">
        ...
        <span className={`${styles.textTitle} font-medium`}>{label}</span>
      </div>
...
export const ToggleItem: React.FC<{
  ...
  return (
    <div className={`flex items-center justify-between py-4 px-4 ${styles.bgCard} ${styles.border} first:rounded-t-xl last:rounded-b-xl border-b last:border-0`}>
      <div className="flex flex-col">
        <span className={`${styles.textTitle} font-medium`}>{label}</span>
        {subLabel && <span className={`text-xs ${styles.textTertiary} mt-1`}>{subLabel}</span>}
      </div>
      <div
        onClick={onToggle}
